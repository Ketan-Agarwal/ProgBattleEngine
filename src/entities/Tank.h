...
class Tank {
public:
  Tank(const int &id, const Position &spawnPosition, Direction facing,
  ...
  int getBulletCooldown(const BulletRange range) const {
    return bulletCooldowns.at(range);
  };
  ...
  void setBulletCooldown(BulletRange range);
  int calculateMatchScore() const;

  // Helper functions

  int getFogStealthAPCost(int k) const { return (10 + k) * 3; }
