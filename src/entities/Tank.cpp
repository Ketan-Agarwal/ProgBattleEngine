...
Tank::Tank(const int &id, const Position &spawnPosition, Direction facing,
           int health, int maxHP, int actionPoints, bool isAlive, bool hasDodge)
    : id(id), spawnPosition(spawnPosition), facing(facing), health(health),
      maxHP(maxHP), actionPoints(actionPoints), isAlive(isAlive),
      hasDodge(hasDodge) {
...
void Tank::setBulletCooldown(BulletRange range) {
  switch (range) {
  case BulletRange::SHORT:
    bulletCooldowns[BulletRange::SHORT] = SHORT_BULLET_COOLDOWN;
  case BulletRange::MEDIUM:
    bulletCooldowns[BulletRange::MEDIUM] = MEDIUM_BULLET_COOLDOWN;
  case BulletRange::LONG:
    bulletCooldowns[BulletRange::LONG] = LONG_BULLET_COOLDOWN;
  }
}
...
int Tank::calculateMatchScore() const {
  ...
  // TODO: if a person kills a person, will there be 300 damage score and 10
  // kill score? doesn't make sense
  score += damageDealt;
