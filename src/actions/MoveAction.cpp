...
bool MoveAction::validate (const GameState& state, const Tank& tank) const {
    ...
    if (!state.getGrid().isValid(newPos) || !state.getGrid().isWalkable(newPos)) return false;
...
bool MoveAction::execute (GameState& state) {
    ...
    if (!tank || !validate(state, *tank)) return false;
...
Position MoveAction::getDirectionVector (Direction dir) const {
    switch(dir){
        case Direction::N:      return Position(0, 1);
        case Direction::E:      return Position(1, 0);
        case Direction::W:      return Position(-1, 0);
        case Direction::S:      return Position(0, -1);
        case Direction::NW:     return Position(-1, 1);
        case Direction::NE:     return Position(1, 1);
        case Direction::SW:     return Position(-1, -1);
        case Direction::SE:     return Position(1, -1);
    }
}